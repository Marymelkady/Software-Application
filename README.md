...
`example` contains a web app, made with React.js, that displays a speedometer in your browser. The app connects to `server` and receives the live data stream, updating the speedometer in real time. This demonstrates to you what a potential application submission could look like.


## Quick explanation

Our Python server uses something called **Server-Sent Events (SSE)**. Clients connect **once**, and it keeps the connection open and pushes you a new message every 50ms, forever, without you asking again.
...
Each message is a JSON object containing three arrays of the same length. Each index across the three arrays refer to the same sample. Here's an example:
...
1. First, we need to download all the code on this website (repository).` We `strongly` recommend using Git to do this.
...
2. **[OPTIONAL]** We recommend using VSCode to view, edit, and run the code.  and edit the code. If you don't have it, visit [VSCode's official website](https://code.visualstudio.com/download) and follow the instructions there to install it.
...
4. We need to setup a virtual environment. This is a convenient way to download and use all the external libraries we need without affecting your other work.
...
Keep in mind that this entire setup is meant to run on localhost (your own computer). We would like your widget to be a separate application from the server, connected locally through a port (8081 in the example command above). When you are ready to run your widget, you should first startup the server, then startup your widget.
...
```shell
# If your terminal isn't already in the example folder, make sure you are in it
cd example

# Now, install all the dependencies for the example widget
npm install

# Finally, run the example widget. Remember to start the server first!
npm run dev
```
